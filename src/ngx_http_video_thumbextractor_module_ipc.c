...
ngx_http_video_thumbextractor_fork_extract_process(ngx_uint_t slot)
{
    ...
    default:
        /* parent */
        ipc_ctx->pid = pid;
        ipc_ctx->conn = ngx_get_connection(socks[0], ngx_cycle->log);
...
ngx_http_video_thumbextractor_extract_process_child_read_handler(ngx_event_t *ev)
{
    ...
    if (transfer->pool == NULL) {
        ...
        transfer->step = NGX_HTTP_VIDEO_THUMBEXTRACTOR_TRANSFER_SECOND;
        ngx_http_video_thumbextractor_set_buffer(&transfer->buffer, (u_char *) &thumb_ctx->second, NULL, sizeof(ngx_int_t));
    }
    ...
    switch (transfer->step) {
    case NGX_HTTP_VIDEO_THUMBEXTRACTOR_TRANSFER_SECOND:
        ngx_http_video_thumbextractor_set_buffer(&transfer->buffer, (u_char *) &thumb_ctx->width, NULL, sizeof(ngx_int_t));
        transfer->step = NGX_HTTP_VIDEO_THUMBEXTRACTOR_TRANSFER_WIDTH;
        break;

    case NGX_HTTP_VIDEO_THUMBEXTRACTOR_TRANSFER_WIDTH:
        ngx_http_video_thumbextractor_set_buffer(&transfer->buffer, (u_char *) &thumb_ctx->height, NULL, sizeof(ngx_int_t));
        transfer->step = NGX_HTTP_VIDEO_THUMBEXTRACTOR_TRANSFER_HEIGHT;
        break;

    case NGX_HTTP_VIDEO_THUMBEXTRACTOR_TRANSFER_HEIGHT:
        ngx_http_video_thumbextractor_set_buffer(&transfer->buffer, (u_char *) &thumb_ctx->filename.len, NULL, sizeof(size_t));
        transfer->step = NGX_HTTP_VIDEO_THUMBEXTRACTOR_TRANSFER_FILENAME_LENGTH;
        break;

    case NGX_HTTP_VIDEO_THUMBEXTRACTOR_TRANSFER_FILENAME_LENGTH:
        ...
        transfer->step = NGX_HTTP_VIDEO_THUMBEXTRACTOR_TRANSFER_FILENAME_DATA;
        break;

    case NGX_HTTP_VIDEO_THUMBEXTRACTOR_TRANSFER_FILENAME_DATA:
        ngx_http_video_thumbextractor_set_buffer(&transfer->buffer, (u_char *) &thumb_ctx->file_info.offset, NULL, sizeof(off_t));
        transfer->step = NGX_HTTP_VIDEO_THUMBEXTRACTOR_TRANSFER_FILE_OFFSET;
        break;

    case NGX_HTTP_VIDEO_THUMBEXTRACTOR_TRANSFER_FILE_OFFSET:
...
ngx_http_video_thumbextractor_start_extract(ngx_uint_t slot)
{
    ...
    transfer->thumb_ctx.second = thumb_ctx->second;
    transfer->thumb_ctx.width = thumb_ctx->width;
    transfer->thumb_ctx.height = thumb_ctx->height;
    transfer->thumb_ctx.filename = thumb_ctx->filename;
    transfer->thumb_ctx.file_info.offset = thumb_ctx->file_info.offset;
    transfer->vtlcf = ngx_http_get_module_loc_conf(ctx->request, ngx_http_video_thumbextractor_module);

    transfer->step = NGX_HTTP_VIDEO_THUMBEXTRACTOR_TRANSFER_SECOND;
    ngx_http_video_thumbextractor_set_buffer(&transfer->buffer, (u_char *) &transfer->thumb_ctx.second, NULL, sizeof(ngx_int_t));

    wev = ipc_ctx->conn->write;
